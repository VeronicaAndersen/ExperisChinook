...
phone number and email. Whit limit & offsets*/
  ...
  /* 5. Inserts a new customer to the database.*/
  @Override
  public void insert(Customer customer) {
    String sql = "INSERT INTO customer (first_name, last_name, country, postal_code, phone, email) VALUES (?,?,?,?,?,?)";

    try (Connection conn = DriverManager.getConnection(url, username, password)) {
      sqlConnection(customer, sql, conn);
  ...
  public CustomerSpender customerThatSpendsMost() {
    ...
    String sql = "SELECT invoice.invoice_id, customer.first_name, invoice.total, customer.customer_id, " +
            "customer.last_name, customer.country, customer.postal_code, customer.phone, customer.email " +
...
means the genre that corresponds to the most tracks from invoices associated to that customer.*/
  ...
  /**
   * Method for result Set with SQL Exceptions.
   * @param customerList sets data from sql query into list.
   * @throws SQLException throws error if error occurs.
   */
  private void resultSet(List<Customer> customerList) throws SQLException {
    Customer customer;
    ResultSet resultSet = preparedStatement.executeQuery();
    while (resultSet.next()) {
      customer = new Customer(
  ...
  /**
   * Method for sql connection with SQL Exceptions.
   * @param customer sets a value to sql query that will be executed.
   * @param sql takes the sql query that will be executed.
   * @param conn the connection between the application and database.
   * @throws SQLException throws error if error occurs.
   */
  private void sqlConnection(Customer customer, String sql, Connection conn) throws SQLException {
    preparedStatement = conn.prepareStatement(sql);
